...
export function isPrivilegedRole(
  role?: string | null
): role is Exclude<AdminRole, "user"> {
...
export async function getCurrentAdmin(): Promise<{
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id,email,full_name,plan,subscription_status,subscription_current_period_end,created_at,updated_at,admin_role,creator_verified,moderation_status,last_active_at"
    )
    .eq("id", user.id)
    .maybeSingle();
