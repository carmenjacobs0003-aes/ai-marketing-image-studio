...
export default function LoginPage({ searchParams }: { searchParams: { error?: string; message?: string; redirectTo?: string } }) {
    ...
    <main className="aurora-shell flex min-h-screen items-center justify-center px-4 py-10 text-white sm:px-6">
      <form action={signInWithPassword} className="glass-card relative z-10 w-full max-w-md space-y-6 p-6 sm:p-8">
        ...
        <p className="text-center text-sm text-slate-300">
          New here? <Link className="font-semibold text-cyan-300 hover:text-cyan-200" href={`/signup?redirectTo=${encodeURIComponent(redirectTo)}`}>Create an account</Link>
        </p>
      </form>
    </main>
  );
