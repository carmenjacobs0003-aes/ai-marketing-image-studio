...
export default function SignupPage({ searchParams }: { searchParams: { error?: string; redirectTo?: string } }) {
  ...
  return (
    <main className="aurora-shell flex min-h-screen items-center justify-center px-4 py-10 text-white sm:px-6">
      <form action={signUpWithPassword} className="glass-card relative z-10 w-full max-w-md space-y-6 p-6 sm:p-8">
        <div className="space-y-3 text-center sm:text-left">
          ...
          <p className="text-sm leading-6 text-slate-300">Launch a premium black-glass workspace for AI visuals, copy, brand systems, projects, and billing.</p>
        </div>
        ...
        <label className="block space-y-2 text-sm font-medium">
          <span>Name</span>
          <input autoComplete="name" className="w-full rounded-2xl border px-4 py-3 outline-none ring-cyan-300 hover:border-cyan-300/40 focus:border-cyan-300/70 focus:ring-2" name="fullName" type="text" required />
        </label>
        <label className="block space-y-2 text-sm font-medium">
          <span>Email</span>
        ...
        <label className="block space-y-2 text-sm font-medium">
          <span>Password</span>
          <input autoComplete="new-password" className="w-full rounded-2xl border px-4 py-3 outline-none ring-cyan-300 hover:border-cyan-300/40 focus:border-cyan-300/70 focus:ring-2" name="password" type="password" minLength={8} required />
        </label>
