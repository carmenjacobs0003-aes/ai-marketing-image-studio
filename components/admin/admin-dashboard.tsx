...
export function AdminDashboard({
  ...
  const filteredUsers = data.users.filter((user) => {
    ...
    const matchesStatus =
      ...
      user.admin_role === status;
    return matchesQuery && matchesStatus;
  });
  const maxDaily = Math.max(
    ...data.dailyGenerations.map((point) => point.total),
    1
  );
      ...
      <div className="page-container">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
                Secure operator dashboard for analytics, subscriptions, user
                management, moderation, audit trails, and generation reliability
                monitoring.
              </p>
            </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
                ...
                >
                  <div className="flex h-52 w-full items-end justify-center rounded-t-2xl bg-white/[0.035] p-1">
                    <div
                      ...
                      style={{
                        height: `${Math.max((point.total / maxDaily) * 100, point.total ? 8 : 2)}%`
                      }}
          ...
          <div className="glass-card p-5 sm:p-6">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <HealthPill
                label="Success"
                value={`${data.generationHealth.successRate}%`}
              />
              <HealthPill
                label="Errors"
                value={`${data.generationHealth.errorRate}%`}
                danger={data.generationHealth.errorRate > 10}
              />
              <HealthPill
                label="Queued"
                value={String(data.generationHealth.queued)}
              />
              <HealthPill
                label="Processing"
                value={String(data.generationHealth.processing)}
              />
            </div>
            <div className="mt-5 space-y-3">
              {data.generationHealth.recentErrors.length ? (
                data.generationHealth.recentErrors.map((error) => (
                  <div
                    className="rounded-2xl border border-rose-300/20 bg-rose-300/[0.06] p-3"
                    key={`${error.kind}-${error.id}`}
                  >
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-rose-100">
                      {error.kind} error · {dateTime(error.created_at)}
                    </p>
                    <p className="mt-1 line-clamp-2 text-sm text-slate-300">
                      {error.error_message ?? error.prompt}
                    </p>
                  </div>
                ))
              ) : (
                <p className="empty-state text-sm">
                  No generation errors detected in the latest sample window.
                </p>
              )}
            </div>
          </div>
        ...
        <section className="glass-card p-5 sm:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-white">
                Accounts, subscriptions, verification
              </h2>
            </div>
            <form
            ...
            <table className="min-w-[980px] w-full border-separate border-spacing-y-3 text-left text-sm">
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  <tr
                    className="rounded-3xl bg-white/[0.035] ring-1 ring-white/10"
                    key={user.id}
                  >
                    <td className="rounded-l-3xl p-4">
                      ...
                      <p className="text-xs text-slate-400">
                        Joined {shortDate(user.created_at)} · Active{" "}
                    ...
                    <td className="rounded-r-3xl p-4">
                      ...
                      >
                        <input
                          id={`admin-user-id-${user.id}`}
                          type="hidden"
                          name="userId"
                          value={user.id}
                        />
                        <select
                          id={`admin-plan-${user.id}`}
                          className="field-control max-w-28 py-2 text-xs"
                          name="plan"
                        ...
                        <select
                          id={`admin-subscription-status-${user.id}`}
                          className="field-control max-w-36 py-2 text-xs"
                        ...
                        <select
                          ...
                          className="field-control max-w-32 py-2 text-xs"
                          name="adminRole"
                        ...
                        <input
                          id={`admin-creator-verified-${user.id}`}
                          type="hidden"
                          name="creatorVerified"
                          value={user.creator_verified ? "false" : "true"}
                        />
                        <button
                        ...
                        <button
                          className="ghost-button px-3 py-2 text-xs"
                          name="intent"
                          value="toggleVerification"
                          type="submit"
                        >
                          {user.creator_verified ? "Unverify" : "Verify"}
                        </button>
                      </form>
                    </td>
                  </tr>
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <Panel
            ...
            eyebrow="Reports"
            icon={<AlertTriangle className="h-6 w-6 text-cyan-300" />}
          >
            <div className="space-y-3">
              {data.reports.map((report) => (
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="font-black text-white">
                        {report.gallery_item?.title ?? "Deleted content"}
                      </p>
                  ...
                  >
                    <input
                      id={`admin-report-id-${report.id}`}
                      type="hidden"
                      name="reportId"
                      value={report.id}
                    />
                    <input
                      id={`admin-report-item-id-${report.id}`}
                      type="hidden"
                      name="itemId"
                      value={report.gallery_item_id}
                    />
                    <button
                      className="ghost-button px-3 py-2 text-xs"
                      name="intent"
                    ...
                    <button
                      className="ghost-button px-3 py-2 text-xs text-rose-100"
                      name="intent"
                      value="remove"
                      type="submit"
                    >
                ...
                >
                  ...
                  >
                    ...
                    <button
                      className="ghost-button px-3 py-2 text-xs"
                      name="intent"
                      value={item.featured ? "unfeature" : "feature"}
                      type="submit"
                    >
                    ...
                    <button
                      className="ghost-button px-3 py-2 text-xs"
                      name="intent"
                      value={item.visibility === "public" ? "hide" : "restore"}
                      type="submit"
                    >
                      <EyeOff className="mr-1 h-3.5 w-3.5" />
                      {item.visibility === "public" ? "Hide" : "Restore"}
                    </button>
                    <button
                      className="ghost-button px-3 py-2 text-xs text-rose-100"
                      name="intent"
        ...
        <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
          <Panel
            ...
            icon={<HeartPulse className="h-6 w-6 text-cyan-300" />}
          >
          ...
          <Panel
            ...
            eyebrow="Alerts & recovery"
            icon={<AlertTriangle className="h-6 w-6 text-cyan-300" />}
          >
            <div className="space-y-3">
              {data.monitoringIncidents.length ? (
                data.monitoringIncidents.map((incident) => (
                  <div
                    className={cn(
                ...
                <p className="empty-state text-sm">
                  All production diagnostics are clear. {BRAND_NAME} monitoring
                  is ready.
                </p>
              )}
            </div>
          ...
          >
            <div className="space-y-3">
              {data.auditLogs.map((log) => (
                <div
                  className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.035] p-3 sm:flex-row sm:items-center sm:justify-between"
                  key={log.id}
                >
                  <div>
                  ...
                  <Badge>
                    {log.severity} · {dateTime(log.created_at)}
                  </Badge>
                </div>
              ))}
              {!data.auditLogs.length ? (
                <p className="empty-state text-sm">
                  No audit events available. Admin actions will appear here.
                </p>
              ) : null}
            </div>
          </Panel>
  ...
  return (
    <div
      className={cn(
        ...
        danger
          ? "border-rose-300/30 bg-rose-300/[0.07]"
