...
export function PublishGalleryButton({ sourceId, kind, defaultTitle, defaultPrompt }: { sourceId: string; kind: "image" | "marketing"; defaultTitle: string; defaultPrompt: string }) {
  const [open, setOpen] = useState(false);
  const [title, setTitle] = useState(defaultTitle.slice(0, 120));
  const [description, setDescription] = useState("");
  const [category, setCategory] = useState(kind === "image" ? "AI Image" : "Marketing");
      ...
      {open ? (
        <form className="space-y-3 rounded-2xl border border-cyan-300/20 bg-black/70 p-3" onSubmit={onSubmit}>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <select className="field-control py-2" onChange={(event) => setVisibility(event.target.value as "public" | "private")} value={visibility}>
